...
airport_gz_flights = pd.read_csv(r'.\data\airport_gz_flights_chusai_2ndround.csv')#航班信息数据
...
coor_realation = pd.read_csv(r'.\data\coor_realation.csv')#关联性数据表
...
def couterCal(minutes):
    if minutes <10:
        ...
        section=5
    return section
#处理航班信息数据，十分钟，时间，关联登机口数据，去重，并存储。
...
dataTrain= pd.read_csv(r'./data/airport_gz_WiFi_apAfter.csv')#业务流量数据读取
...
for date in dateWindow:
    ...
    dataFeatureOffline = pd.merge(flights,dataLabelUsed,how='outer',on=["WIFIAPTag","time"]).fillna(0)
...
#---featureCombine
dataFeatureOfflineTrain.to_csv(r'./data/dataFeatureOfflineTestSpace.csv',index=False)#训练数据存取
